...
class _HomeScreenNearbyState extends State<HomeScreenNearby> {
  ...
  callApi({double? latitude, double? longitude}) async {
    ...
    print("__________________________" + jsonResponse.toString());
    if (response.statusCode == 200 && jsonResponse['status'] == 1) {
      ...
      if (mounted) {
        setState(() {
          ...
          print("Finished");
          // Filter the list to exclude items with the given id (e.g., id = 20)
          List<NearbyData> filteredList = nearbyDoctorsClass!.data!.nearbyData!.where((data) => data.id != 20).toList();
